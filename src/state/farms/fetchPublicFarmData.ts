...
type PublicFarmData = {
  ...
  depositFee: string
  withdrawFee: string
  squadPerBlock: SerializedBigNumber
}
...
const fetchFarm = async (farm: Farm): Promise<PublicFarmData> => {
  ...
  const [info, totalAllocPoint, squadPerBlock] =
    pid || pid === 0
  ...
  const ChronicPerBlock = squadPerBlock ? new BigNumber(squadPerBlock).div(BIG_TEN.pow(18)) : BIG_ZERO
  ...
  return {
    ...
    squadPerBlock: ChronicPerBlock.toJSON(),
  }
}
